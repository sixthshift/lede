...
const built = await parallel(tickets.map(t => () =>
  agent(
    [
      ...
      '(4) COMMIT your work onto your worktree branch — `git add` the declared',
      '    files + manifest, then `git commit`. UNCOMMITTED work cannot be merged;',
      '    a clean `git status` (aside from ignored build/data artifacts) is required',
      '    before you report done, and report the branch that now holds the commit.',
      '',
...
const merge = passed.length === 0
  ...
  : await agent(
      [
        ...
        'AFTER merging, run the project install command (bun install) in the working',
        'tree so any newly-merged manifest deps are actually present — otherwise the',
        'Gate that follows cannot resolve them. Verify the merged files are really on',
        'the working branch (git log / ls) before returning.',
