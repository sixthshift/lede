...
describe("resolveModel — dispatch correctness", () => {
  const cases: { provider: ProviderId; model: string; extra?: Record<string, string> }[] = [
    ...
    { provider: "google", model: "gemini-2.5-flash" },
    {
      ...
      model: "llama3",
      extra: { baseURL: "http://localhost:11434/v1" },
    },
  ];

  it.each(cases)("builds a model for $provider matching the requested provider+model", ({
    provider,
    model,
    extra,
  }) => {
  ...
  it("does not confuse openai-compatible with plain openai", () => {
    const openai = resolveModel({ provider: "openai", model: "gpt-5", apiKey: "dummy-key" }) as {
      provider: string;
    };
