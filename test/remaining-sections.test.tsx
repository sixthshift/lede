...
describe("remaining sections — coverage (§10)", () => {
  it("renders one entry of each remaining section under its registry label", () => {
    const entries = [
      entry("award1", "award", { section: "award", title: "Employee of the Year", issuer: "Acme" }, ["Recognized for shipping"], 202301),
      entry("cert1", "certification", { section: "certification", name: "AWS Certified Solutions Architect", issuer: "AWS" }, ["AWS Certified Solutions Architect"], 202401),
      entry("pub1", "publication", { section: "publication", title: "Scaling Systems", venue: "ACM" }, ["Scaling Systems"], 202101),
      ...
      entry("ref1", "reference", { section: "reference", name: "Jane Smith", relationship: "Manager" }, [], 202201),
    ];
    const layout = layoutFor(["award", "certification", "publication", "interest", "language", "reference"]);
    const resume = assemble(decisionFor(entries), entries, layout);
    render(<ResumePage resume={resume} />);

    for (const section of ["award", "certification", "publication", "interest", "language", "reference"] as const) {
...
describe("remaining sections — meta fallback for empty facts (§4.3, gameable-resistant)", () => {
  it("renders a certification with empty facts from its meta, not blank", () => {
    const entries = [
      entry("cert2", "certification", { section: "certification", name: "AWS Certified Cloud Practitioner", issuer: "Amazon Web Services" }, [], 202401),
    ];
  ...
  it("renders a reference with empty facts from its meta, not blank", () => {
    const entries = [
      entry("ref2", "reference", { section: "reference", name: "Jane Smith", relationship: "Manager" }, [], 202201),
    ];
  ...
  it("still renders facts verbatim when present (no regression for rephrase:'none')", () => {
    const entries = [
      entry("cert3", "certification", { section: "certification", name: "Should Not Appear" }, ["Verbatim Fact Text"], 202401),
    ];
    const layout = layoutFor(["certification"]);
    const resume = assemble(decisionFor(entries), entries, layout);
    render(<ResumePage resume={resume} />);
  ...
  it("leaves rephrase:'full'/'light' sections rendering the model's text unchanged", () => {
    const entries = [
      entry("exp1", "experience", { section: "experience", company: "Acme", role: "Eng", period: "2020-2023" }, ["did a thing"], 202301),
    ];
