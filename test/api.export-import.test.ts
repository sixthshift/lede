...
describe("RED-TEAM #3: round-trip is non-vacuous — export, wipe, import, re-export deep-equals", () => {
  ...
  // E9-F2d: the structured group header (headingParts, the date-format axis)
  // must survive export -> import, not be silently stripped by tailoredResumeZ
  // (a plain z.object drops unknown keys) — §27 "snapshots round-trip intact".
  it("a snapshot's headingParts survives export -> import intact (not stripped by the import validator)", async () => {
    const structuredResume: TailoredResume = {
      signals: { roleLevel: "staff", weights: [], hardRequirements: [] },
      summary: "Structured-header snapshot.",
      sections: [
        {
          section: "experience",
          groups: [
    ...
    sourceDb
      .insert(applications)
      .values({ ...seededApplication(), current: structuredResume, locked: structuredResume })
      .run();
    const sourceApp = buildApp(sourceDb);
    await sourceApp.inject({
      method: "PUT",
    ...
    const backup = (await sourceApp.inject({ method: "GET", url: "/api/export" })).json();
    expect(backup.applications[0].current).toEqual(structuredResume);
