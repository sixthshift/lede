// 'classic' template — single-column, ATS-strict composition (spec.md §28.2).
...
// Distinguishing composition vs. strict: a centered profile header (name +
// contact line both centered) — the hairline rule under each section heading
// is already the shared SectionBlock's sectionLabel treatment.
...
export function ClassicTemplate({ resume, profile, paper, format }: TemplateProps) {
  ...
  return (
    <Document title={profile.name} author={profile.name}>
      <Page size={PAGE_SIZE[paper]} style={styles.page}>
        <ProfileHeader profile={profile} format={format} variant="centered" />
